...
2\. YARA ルールを使用する場合は `yara\_rules/` フォルダを exe と同じ階層に配置

3\. `Sacabambaspis.exe` を管理者として実行

4\. ブラウザが自動で開きます（デフォルト: `http://localhost:5000`）
...
3\. または手動で `yara\_rules/` フォルダに `.yar` / `.yara` ファイルを配置
...
\- `yara\_rules/` は同梱しない（Defender 誤検知回避）
...
├── collectors/                # 16 コレクタモジュール

│   ├── process.py             # プロセス解析

│   ├── memory.py              # メモリ情報

│   ├── memdump.py             # メモリダンプ・PE 抽出・文字列解析

│   ├── network.py             # ネットワーク接続

│   ├── file\_inspector.py      # ファイル構造解析

│   ├── registry.py            # レジストリ検査

│   ├── persistence.py         # 永続化検出
...
│   ├── yara\_manager.py        # YARA ルール管理

│   └── path\_helper.py         # パス解決（exe/通常実行対応）

├── yara\_rules/                # YARA ルール（exe 非同梱）
...
| YARA スキャンでマッチなし | ルール未配置 | yara\_rules/ フォルダを配置 |

| ブラウザが開かない | ポート競合 | 別のポートを指定（main.py の PORT 変数） |

| 文字化けー | エンコーディング | ブラウザの文字コードを UTF-8 に設定 |
...
\- \*\*リポジトリ\*\*: \[GitHub (Private)](https://github.com/matrix9neonebuchadnezzar2199-sketch/Sacabambaspis)

\- \*\*技術スタック\*\*: Python 3.13 + Flask + JavaScript (ES5) + HTML/CSS
...
print(f"README.md created: {len(content.splitlines())} lines")

'@



$pyFile = "F:\\Sacabambaspis\\\_tmp\_readme.py"
...
$lines = (Get-Content "F:\\Sacabambaspis\\README.md" -Encoding UTF8).Count

Write-Host "README.md: $lines lines" -ForegroundColor Cyan
