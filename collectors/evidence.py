...
class EvidenceCollector:
    ...
    def _analyze_userassist(self, decoded_name, run_count):
        """UserAssistエントリの危険度を判定し、3段構成の解説を生成"""
        ...
        for tool in self.attack_tools:
            if tool in name_lower:
                return (
                    "DANGER",
                    f"攻撃ツール検知: {tool}",
                    f"【検知内容】UserAssistレジストリに攻撃ツール「{tool}」の実行記録が残っています。"
                    f"実行回数: {run_count}回\n"
                    f"パス: {decoded_name}\n\n"
                    f"【なぜ危険か】UserAssistはエクスプローラ経由で実行されたプログラムの履歴を"
                    f"ROT13エンコードで記録するレジストリキーです。"
                    f"攻撃者がGUI操作でツールを実行した証拠であり、"
                    f"侵害が対話的（ハンズオンキーボード）段階に進んでいることを示します。\n\n"
                    f"【次の調査手順】\n"
                    f"① Prefetchで同ファイルの実行時刻を照合する\n"
                    f"② 同時刻帯に実行された他のプログラムを確認する\n"
                    f"③ ファイルのハッシュ値を取得し、VirusTotal等で検索する\n"
                    f"④ イベントログ(Security:4688)でコマンドライン引数を確認する\n\n"
                    f"【MITRE ATT&CK】T1218 - System Binary Proxy Execution\n"
                    f"https://attack.mitre.org/techniques/T1218/"
                )
        ...
        for lolbin in self.lolbins:
            ...
            if exe_name == lolbin or exe_name == lolbin_base or exe_name.startswith(lolbin_base + '.'):
                return (
                    "WARNING",
                    f"LOLBin実行: {lolbin}",
                    f"【検知内容】UserAssistにLiving-off-the-Land Binary「{lolbin}」の"
                    f"実行記録があります。実行回数: {run_count}回\n"
                    f"パス: {decoded_name}\n\n"
                    f"【なぜ危険か】LOLBin（Living-off-the-Land Binary）はWindowsに標準搭載された"
                    f"正規ツールですが、攻撃者がマルウェアのダウンロード・実行・横展開に"
                    f"悪用するケースが非常に多いです。"
                    f"正規の管理作業で使用される場合もあるため、実行コンテキストの確認が必要です。\n\n"
                    f"【次の調査手順】\n"
                    f"① イベントログ(Security:4688, PowerShell:4104)でコマンドライン引数を確認する\n"
                    f"② 実行時刻が業務時間内か確認する\n"
                    f"③ 同時刻帯のネットワーク接続(netstat)を確認する\n"
                    f"④ 正規の管理作業として説明がつくか、担当者に確認する"
                )
        ...
        for recon in self.recon_tools:
            if recon in exe_name:
                return (
                    "WARNING",
                    f"偵察コマンド実行: {recon}",
                    f"【検知内容】UserAssistに偵察コマンド「{recon}」の実行記録があります。"
                    f"実行回数: {run_count}回\n"
                    f"パス: {decoded_name}\n\n"
                    f"【なぜ危険か】攻撃者は侵害後の初期段階で、ネットワーク構成・ユーザー情報・"
                    f"ドメイン情報を収集する「偵察活動」を行います。"
                    f"whoami、systeminfo、net user 等のコマンドが短時間に集中して実行された場合、"
                    f"攻撃者の手動偵察（Discovery フェーズ）の可能性があります。\n\n"
                    f"【次の調査手順】\n"
                    f"① 同一時間帯（前後30分）に他の偵察コマンドが実行されていないか確認する\n"
                    f"② 実行ユーザーアカウントが正当な管理者か確認する\n"
                    f"③ イベントログでコマンドライン引数と出力先を確認する\n"
                    f"④ 3つ以上の偵察コマンドが集中していれば、インシデント対応を検討する\n\n"
                    f"【MITRE ATT&CK】T1087 - Account Discovery\n"
                    f"https://attack.mitre.org/techniques/T1087/"
                )
        ...
        for susp_path in self.suspicious_paths:
            if susp_path in name_lower:
                return (
                    ...
                    f"【検知内容】UserAssistに不審なフォルダから実行されたプログラムの記録があります。"
                    f"実行回数: {run_count}回\n"
                    f"パス: {decoded_name}\n\n"
                    f"【なぜ危険か】Temp、Downloads、Publicフォルダはユーザー権限で書き込めるため、"
                    f"攻撃者がマルウェアを配置する定番の場所です。"
                    f"正規のインストーラが一時的に使用する場合もありますが、"
                    f"見覚えのないEXEファイルが実行されていた場合は調査が必要です。\n\n"
                    f"【次の調査手順】\n"
                    f"① 該当パスにファイルがまだ存在するか確認する\n"
                    f"② 存在する場合、ファイルのハッシュ値をVirusTotalで検索する\n"
                    f"③ Zone.Identifier(ADS)でダウンロード元URLを確認する\n"
                    f"④ 削除済みの場合、Prefetch/PCAで実行時刻を特定する"
                )
    ...
    def _scan_prefetch(self):
        ...
        if not os.path.exists(prefetch_dir):
            results.append({
                ...
                "desc": (
                    "【検知内容】C:\\Windows\\Prefetch フォルダが存在しません。\n\n"
                    "【なぜ危険か】Prefetchはプログラムの起動を高速化するためのキャッシュで、"
        ...
        try:
            ...
            results.append({
                ...
                "reason": "管理者権限が必要です",
                "desc": (
                    "【検知内容】C:\\Windows\\Prefetch フォルダへのアクセスが拒否されました。\n\n"
                    "【なぜ危険か】Prefetchフォルダの読み取りには管理者権限が必要です。"
    ...
    def _analyze_prefetch(self, filename, dt):
        """Prefetchファイル名から危険度を判定し、3段構成の解説を生成"""
        ...
        for tool in self.attack_tools:
            if tool in exe_lower:
                return (
                    "DANGER",
                    f"攻撃ツール実行痕跡: {tool}",
                    f"【検知内容】Prefetchに攻撃ツール「{tool}」の実行キャッシュが残っています。\n"
                    f"ファイル: {filename}\n"
                    f"最終実行: {timestamp_str}\n\n"
                    f"【なぜ危険か】Prefetchはプログラム実行時にOSが自動作成するキャッシュファイルで、"
                    f"プログラム本体が削除されても実行の証拠として残り続けます。"
                    f"攻撃ツールのPrefetchが存在するということは、"
                    f"過去にこのPCで当該ツールが実行されたことを意味します。\n\n"
                    f"【次の調査手順】\n"
                    f"① Prefetch解析ツール(PECmd等)で詳細な実行時刻とロードしたDLLを確認する\n"
                    f"② 同時刻帯に他の攻撃ツールが実行されていないか確認する\n"
                    f"③ UserAssist/PCA/CAM DBと照合して実行パスを特定する\n"
                    f"④ イベントログ(Security:4688)で実行ユーザーとコマンドラインを確認する\n\n"
                    f"【MITRE ATT&CK】T1588.002 - Obtain Capabilities: Tool\n"
                    f"https://attack.mitre.org/techniques/T1588/002/"
                )
        ...
        for lolbin in self.lolbins:
            ...
            if exe_lower == lolbin_name or exe_lower == lolbin_name + '.exe' or exe_lower.startswith(lolbin_name + '.'):
                return (
                    "WARNING",
                    f"LOLBin実行痕跡: {lolbin}",
                    f"【検知内容】PrefetchにLOLBin「{lolbin}」の実行キャッシュがあります。\n"
                    f"ファイル: {filename}\n"
                    f"最終実行: {timestamp_str}\n\n"
                    f"【なぜ危険か】LOLBin自体はWindows標準ツールですが、"
                    f"攻撃者がファイルレス攻撃やダウンローダとして悪用するケースが多く、"
                    f"Prefetchからはコマンドライン引数が分からないため、"
                    f"イベントログとの照合で正規利用か攻撃利用かを判断する必要があります。\n\n"
                    f"【次の調査手順】\n"
                    f"① イベントログ(Security:4688)で同時刻のコマンドライン引数を確認する\n"
                    f"② PowerShell:4104でスクリプトブロックログを確認する\n"
                    f"③ 正規の管理作業として説明がつくか確認する"
                )
        ...
        for recon in self.recon_tools:
            ...
            if exe_lower == recon_name or exe_lower == recon_name.replace('.exe', '') + '.exe' or exe_lower.startswith(recon_name + '.') or exe_lower.startswith(recon_name + '_'):
                return (
                    "WARNING",
                    f"偵察コマンド痕跡: {recon}",
                    f"【検知内容】Prefetchに偵察コマンド「{recon}」の実行キャッシュがあります。\n"
                    f"ファイル: {filename}\n"
                    f"最終実行: {timestamp_str}\n\n"
                    f"【なぜ危険か】偵察コマンド単体では必ずしも危険とは限りませんが、"
                    f"複数の偵察コマンドが短時間に集中して実行されている場合、"
                    f"攻撃者による環境調査（MITRE ATT&CK: Discovery）の可能性があります。\n\n"
                    f"【次の調査手順】\n"
                    f"① 同時刻帯（前後1時間）に他の偵察コマンドのPrefetchがないか確認する\n"
                    f"② 3つ以上の偵察コマンドが集中していればインシデント対応を検討する\n"
                    f"③ 実行ユーザーと実行時間帯（業務時間内/外）を確認する\n\n"
                    f"【MITRE ATT&CK】T1087 - Account Discovery\n"
                    f"https://attack.mitre.org/techniques/T1087/"
                )
        ...
        return (
            "INFO",
            "",
            f"プログラム実行キャッシュです。最終実行: {timestamp_str}"
        )
